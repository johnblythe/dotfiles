#!/bin/bash
#
# Generate a questionnaire for a service (no Claude CLI required for SME)
#
# Usage:
#   ./generate-questionnaire.sh intakeservices
#   # Sends questionnaire to SME
#   # SME fills it out, returns it
#   ./process-questionnaire.sh intakeservices-questionnaire-filled.md
#
...
OUTPUT_DIR="$SCRIPT_DIR/questionnaires"
...
OUTPUT_FILE="$OUTPUT_DIR/${SERVICE_NAME}-questionnaire-${TIMESTAMP}.md"

cat > "$OUTPUT_FILE" << 'QUESTIONNAIRE'
...
sed -i '' "s/DATE_PLACEHOLDER/$TIMESTAMP/g" "$OUTPUT_FILE"

echo "✅ Questionnaire generated: $OUTPUT_FILE"
echo ""
echo "Next steps:"
echo "1. Send this file to the SME for $SERVICE_NAME"
echo "2. They fill it out (any text editor works)"
echo "3. Run: ./process-questionnaire.sh <filled-questionnaire.md>"
