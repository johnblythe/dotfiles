-- Q39: Best sites exception reason distribution vs population
with site_stats as (
    select
        e.dds_site_id,
...
best_sites as (
    ...
    order by (rework_cnt + exception_cnt) * 1.0 / total_requests asc
    limit 5
)
select
    'BEST_5' as cohort,
...
where e.dds_site_id in (select dds_site_id from best_sites)
